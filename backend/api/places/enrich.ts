...
async function detectReservationSource(
  ...
  // Try Resy first
  try {
    const resyId = await resyLib.findVenueId(name, lat, lng);
    if (resyId) {
      return {
        platform: 'resy',
        venueId: resyId,
        directBookingURL: `https://resy.com/cities/unknown/${encodeURIComponent(name.toLowerCase().replace(/\s+/g, '-'))}`,
      };
    }
  } catch {
    // fall through
  ...
  // Could add OpenTable and Tock venue lookup here
  return undefined;
}
