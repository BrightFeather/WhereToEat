...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  logger.request('GET', '/api/scrape/eater', { city });

  try {
    const items = await scrapeEater(city as string);
    logger.success('eater.scrape.complete', { city, itemCount: items.length });
    return res.status(200).json(ok(items));
  } catch (e: unknown) {
    logger.error('eater.scrape.failed', e, { city });
...
async function scrapeEater(cityInput: string): Promise<EaterItem[]> {
  ...
  if (!slug) {
    logger.warn('eater.scrape.unknown_city', { city: cityInput });
    return [];
  }
