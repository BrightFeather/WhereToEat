...
import { searchAvailability } from '../../_lib/opentable';
...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  logger.request('GET', '/api/reservations/opentable/search', { venueId, dates, partySize });
  ...
  try {
    ...
    logger.success('opentable.search.complete', { venueId, slotCount: slots.length });
    return res.status(200).json(ok({ slots }));
  } catch (e: unknown) {
    const message = e instanceof Error ? e.message : 'OpenTable error';
    logger.error('opentable.search.failed', e, { venueId });
    return res.status(500).json(err(message, 'OPENTABLE_ERROR'));
  }
}
