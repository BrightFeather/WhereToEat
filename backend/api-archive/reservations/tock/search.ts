...
import { searchAvailability } from '../../_lib/tock';
...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  logger.request('GET', '/api/reservations/tock/search', { venueId, dates, partySize });
  ...
  try {
    ...
    logger.success('tock.search.complete', { venueId, slotCount: slots.length });
    return res.status(200).json(ok({ slots }));
  } catch (e: unknown) {
    const message = e instanceof Error ? e.message : 'Tock error';
    logger.error('tock.search.failed', e, { venueId });
    return res.status(500).json(err(message, 'TOCK_ERROR'));
  }
}
