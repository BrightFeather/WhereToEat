...
final class DiscoveryViewModel: ObservableObject {
    ...
    func loadCards() async {
        // Use real location if available, fall back to San Francisco for mock/demo mode
        let coords = locationService.effectiveCoordinates
            ?? CLLocationCoordinate2D(latitude: 37.7749, longitude: -122.4194)
        let city = locationService.effectiveCityName.isEmpty
            ? "San Francisco" : locationService.effectiveCityName
