...
final class DiscoveryService: ObservableObject {
    ...
    func fetchRestaurants(
        ...
        // 2. Mock data (replace with real backend calls once Vercel is deployed)
        all.append(contentsOf: MockData.restaurants)
        ...
        // Rank: custom first, then liked-not-booked, then popularity
        let ranked = rank(filtered, swipeHistory: swipeHistory)
        return DiscoveryResult(restaurants: ranked, errors: [])
    }

    // MARK: - Enrichment

    func enrich(_ restaurant: Restaurant) async -> Restaurant {
        // Skip backend enrichment until Vercel is deployed — mock data is already fully populated
        return restaurant
    }
